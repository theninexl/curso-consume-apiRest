...
const uploadMichiEP = 'https://api.thecatapi.com/v1/images/upload';
...
const saveFavouriteMichi = async (id) => {
    ...
    //esto es lo mismo que con fetch, pero usando axios
    const {data,status} = await axiosApi.post('/favourites',{
        image_id: id
    });
...
const uploadMichi = async ()=> {
    ...
    if (resUF.status != 201) {
        spanError.innerHTML = "Upload de Michi falló con error: " + resUF.status + dataUF.message;
    }else {        
        console.log('Michi Uploaded');
        console.log(dataUF);
        console.log(dataUF.url);
        saveFavouriteMichi(dataUF.id);
        loadFavouriteMichis();
    } 
}
